...
require('colors')
const add = function (userSettings, app, renderType) {
    const actions = app.actions
    if (typeof userSettings.url !== 'string') {
        throw new Error(`
${`app.${renderType}(settings)`.green}
${`settings.url should be a string`.red}
`)
    }
    if (typeof userSettings.type !== 'string') {
        throw new Error(`
${`app.${renderType}(settings)`.green}
${`settings.type should be a string`.red}
`)
    }
    userSettings.__renderType = renderType
